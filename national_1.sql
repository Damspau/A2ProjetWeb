...
CREATE DEFINER=`root`@`localhost` PROCEDURE `PostData` (IN `usernameInput` VARCHAR(30), IN `passwordInput` VARCHAR(150), IN `emailInput` VARCHAR(150), IN `locationInput` VARCHAR(30))  BEGIN 
DECLARE localisationcalc, gradecalc, useridcalc INT;
INSERT INTO utilisateur (username, password, email) VALUES (usernameInput, passwordInput, emailInput);
...
SELECT username FROM utilisateur WHERE email=usernameInput;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UserLogin` (IN `emailInput` VARCHAR(50), IN `passwordInput` VARCHAR(50))  NO SQL
BEGIN 
...
INSERT INTO `utilisateur` (`idUser`, `username`, `password`, `email`) VALUES
